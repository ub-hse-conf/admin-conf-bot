...
if sudo IMAGE_TAG="ghcr.io/ub-hse-conf/cub-admin-telegram-bot:$TARGET_TAG" docker stack deploy -c docker-compose.yml cub-admin-bot --with-registry-auth; then
    echo "✅ Деплой запущен с тегом: $TARGET_TAG"

    # Ждем стабилизации сервисов
    if wait_for_services "cub-admin-bot"; then
    ...
    if sudo IMAGE_TAG="ghcr.io/ub-hse-conf/cub-admin-telegram-bot:stable" docker stack deploy -c docker-compose.yml cub-admin-bot --with-registry-auth; then
